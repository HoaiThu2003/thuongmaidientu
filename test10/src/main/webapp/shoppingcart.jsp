						...
						<c:forEach var="i" begin="0" end="${fn:length(listCart)-1}"
							...
							<div class="product" data-cartid="${listCart[i].id}" data-itemid="${listItem[i].id}">
								...
								<div class="product-info">
									...
									<div class="product-remove"><a href="javascript:void(0);" class="remove-item"
											data-cartid="${listCart[i].id}"
											data-itemid="${listItem[i].id}"> <i class="fa fa-trash"></i>
										</a>
									</div>
				...
				.ready(
						function() {
							...
							$('.quantity-decrease').click(function() {
								var cartID = $(this).data('cartid');
								var itemId = $(this).data('itemid');
								var currentQuantity = parseInt($('#quantity-' + cartID).text());
						        // Kiểm tra nếu số lượng hiện tại đã là 1
											...
											success : function(response) {
												...
												if (response.success) {
													if (action === 'remove') {
														...
														$('#totalPrice-'+ cartID).text(response.newTotalPrice+ ' VND');
														$('#total').text(response.tongtienGiohang+ ' VND');
														$('#totalQuantity').text(response.totalQuantity);
													}
												} else {
